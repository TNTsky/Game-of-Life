...
def update():
    for x in range(CellNum[0]):
        for y in range(CellNum[1]):
            if Cell[x, y]:
                pygame.draw.rect(screen_image, (255, 255, 255), [
                                 x*10, y*10, 10, 10], 5)
            else:
                pygame.draw.rect(screen_image, (0, 0, 0),
                                 [x*10, y*10, 10, 10], 5)
                pygame.draw.rect(screen_image, (100, 100, 100), [
                                 x*10, y*10, 10, 10], 1)
            if [x, y] in pre:
                pygame.draw.rect(screen_image, (160, 160, 160), [
                                 x*10, y*10, 10, 10], 5)
    alive_image = pygame.font.Font(
        "font/msjh.ttc", 24).render("███████████████████", True, (0, 0, 0), (0, 0, 0))
    screen_image.blit(alive_image, (820, 50))
    alive_image = pygame.font.Font(
        "font/msjh.ttc", 24).render(f'存活數量:{len(alive)}', True, (200, 200, 200), (0, 0, 0))
    screen_image.blit(alive_image, (820, 50))
...
play_button = button.Button(900, 740, play_img, 70, screen_image)
...
while True:
    ...
    if play_button.down(screen_image):
        if play:
            ...
            play = True
            play_img = pygame.image.load('images/play_down.png')
        button.Button(900, 740, play_img, 70, screen_image)
